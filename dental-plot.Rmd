...
library(patchwork)
library(here)
dental_inv <- readr::read_csv(here("data", "dental-inv.csv"))
```
...
The two dental plots are combined using the **patchwork** package. We will separate
them using `/` instead of `+`, because we want one on top of the other to mimic
the maxilla's and mandible's position in the mouth:
...
UPDATE: The `plot_layout` function allows us to easily combine the identical
legends. Thanks [@jfy133](https://twitter.com/jfy133) for bringing it to my 
